<template>
  <div class="detail-wrap">
      ...
      <div class="article-info">
        ...
        <div class="time">
          <i class="iconfont icon-rili"
            >Created {{ dayjs(articleData.createTime).format('YYYY-MM-DD') }}</i
          >
          <i class="iconfont icon-rili"
            >Updated {{ dayjs(articleData.updateTime).format('YYYY-MM-DD') }}</i
          >
        </div>
        <div class="action">
          <span
            ><i class="iconfont icon-eye1"></i>Post View:
            {{ articleData.reading }}</span
          >
        </div>
      </div>
    </div>
    <div class="detail-content">
      <div
        class="content-left card markdown-body"
      ...
      const getBlogArticleList = () => {
        getArticleList({ status: 1 }).then((res) => {
          if (res.data.code === 200) {
            ...
            state.articleData.content = marked(rows[0].content).replaceAll(
              '<pre>',
              '<pre class="hljs" style="background: #23241f;">'
            )
          }
        })
      }
