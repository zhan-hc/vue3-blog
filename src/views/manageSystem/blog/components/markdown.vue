...
<script>
  ...
  export default defineComponent({
    ...
    setup() {
      ...
      const { categoryData, getCategoryListFun } = categoryFun()
      const { tagData, getTagListFun } = tagFun()
      ...
      const handleSubmit = () => {
        submitForm(
          ...
          () =>
            state.status
              ? updateBlogArticle({ ...state.ruleForm, id: state.id })
              : addBlogArticle(state.ruleForm),
          (res) => {
        ...
        getArticleList({ id: id }).then((res) => {
          if (res.data.code === 200) {
            ...
            state.ruleForm = {
              title: rows[0].title,
              tagId: rows[0].tagId
                ? rows[0].tagId.split(',').map((item) => Number(item))
                : [],
